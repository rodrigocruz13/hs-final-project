#!/usr/bin/python3

import json
import re
import requests
import sys
import time

from multiprocessing import Pool

now_stamp = __import__('os_functions').now_stamp


def validate_domain(web_address):
    """
    Purpose:   Function that queries a web address to validate connection.
    Arguments: - web_address (str): url to check.
    Returns:   - A URL in correct format if available connection
               - None if no connection available.
    """

    url = web_address
    if (web_address[-1] != '/'):
        url = url + '/'

    try:
        url_request = requests.get(url).text
        return (url)
    except:
        return (None)


def get_link_list(fixed_url_list):
    """
    Purpose:   Gets all the valid and unique links from a fixed url.
    Arguments: - fixed_url (str): Web address used to find the links.
    Returns:   - unique_links (list): A list of unique links (str) of the URL.
    """
    start = time.time()

    print('URL[s]:\t\t{}'.format(fixed_url_list))

    unique_links = []
    for fixed_url in fixed_url_list:
        if (fixed_url is not None):
            html_response = requests.get(fixed_url).text
            links_regex = re.compile(
                '<a\s+.*href=[\'"]?([^\'" >]+)', re.IGNORECASE)
            original_links = links_regex.findall(html_response)
            original_links.sort()
            fixed_links = add_backslash(original_links)

            forbidden = ['?cid', '#disqus_thread',
                         '#', '#disqus_thread', '?',
                         '&amp', 'ref=', '/?']

            for link in fixed_links:
                full_url = re.sub(r'^\.?/{1,2}', fixed_url, link, count=1)

                for word in forbidden:
                    full_url = trim_string(full_url, word)

                if (full_url is not None) and (len(full_url) > 0):
                    if (full_url[-1] == '/'):
                        full_url = full_url[0:-1]

                http = full_url.find('http')
                java = full_url.find('javascript:void(0)')
                brac = full_url.find('{')
                mail = full_url.find('mailto:')

                repeat = 1
                if (full_url not in unique_links):
                    repeat = - 1

                if (http == 0 and (java + brac + mail + repeat == -4)):
                    unique_links.append(full_url)

        if(fixed_url is not None and fixed_url[0:-1] not in unique_links):
            unique_links.append(fixed_url)

        name = trim_string(fixed_url, '://')
        name = trim_string(name, '.')
        name = trim_string(name, '/')

    unique_links.sort()
    end = time.time()

    print("Finding links.  Time in seconds : ", (end-start))
    return(unique_links)


def trim_string(a_string, a_text):
    """
    Purpose:   trim a string until the occurrence of a specific text.
    Arguments: - a_string (str): string to be cut
               - a_text (str): Text to look for inside the string.
    Returns:   - A trimmed str if the text is found or the same string if not.
    """

    if a_string is not None:
        found = a_string.find(a_text)
        if (found != -1):
            return a_string[0: found]
        return a_string
    return None


def add_backslash(a_list):
    """
    Purpose:   trim a string until the occurrence of a specific text.
    Arguments: - a_string (str): string to be cut
               - a_text (str): Text to look for inside the string.
    Returns:   - A trimmed str if the text is found or the same string if not.
    """

    if a_list is not None:
        cont_http = 0
        cont_https = 0
        for link in a_list:
            if link[0].find('https:'):
                cont_https += 1
            if link[0].find('http:'):
                cont_http += 1

        if(cont_https >= cont_http):
            prefijo = 'https:'
        else:
            prefijo = 'http:'

        new_list = []
        for link in a_list:
            if len(link) > 2 and link[0] == '/' and link[1] == '/':
                link = prefijo + link
            new_list.append(link)

    return (new_list)


def check_all(url, link_list):
    """
    Purpose:   Checks the code status of each link.
               Saves the result in a JSON file
    Arguments: - url
               - link_list (lst): List with all the links from an address.
    Returns:   - status_dict (Dict): info generated by checking process.
    """

    start = time.time()
    status_dict = {}
    list_len = len(link_list)
    print('Links found:\t{}'.format(list_len))
    codes_dict = load_dict_from_file('_status_codes')
    code_list = pooling_list(link_list)

    i = 0
    #print(code_list)
    print()

    error_dict = {}
    for link in link_list:

        code_i = code_list[i][0]
        #print(i, code_i, type(code_i))
        date_i = code_list[i][1]

        class_code = codes_dict[str(code_i)[0]]
        explain_code = codes_dict[str(code_i)]

        status_dict_i = {}
        status_dict_i.update({"Code": str(code_i)})
        status_dict_i.update({"Type": class_code})
        status_dict_i.update({"Meaning": explain_code})

        #print('\n',status_dict_i)

        check_dict_i = {}
        id = "Check" + ' ' + str(i + 1).zfill(4)
        check_dict_i.update({"Date": date_i})
        check_dict_i.update({"Link": link})
        check_dict_i.update({"Status": status_dict_i})

        #print('\n',check_dict_i)

        status_dict.setdefault(id, check_dict_i)

        if (code_i > 399):
            error_dict.setdefault(id, check_dict_i)
        i += 1
    end = time.time()
    total_time = end - start
    print("\nCheckin links.  Time in seconds : ", total_time)

    colon = url[0].find(":") + 3
    basename = url[0][colon: -1]

    save_dict_to_file(status_dict, basename)
    error_name = basename + '--error_dict'
    save_dict_to_file(error_dict, error_name)

    return(basename)


def print_results(result_filename):
    """
    Purpose:   Print the results from the result file.
    Arguments: - url (str): Url domain.
               - status_dict (Dict): Dict generated by the checking process.
    Returns:   - Nothing
    """

    a_dict = load_dict_from_file(result_filename)
    dict_lenght = len(a_dict)
    if (dict_lenght == 0 or a_dict is None):
        print('\nNothing to report')
        exit(1)

    print('\n{} checks done\n'.format(dict_lenght))
    opt = print_menu_results(result_filename)
    return (opt)


def print_board(basename):
    """
    Purpose:   Print a board with the results.
    Arguments: - basename (str): name to use when saving the error dict.
    Returns:   - Nothing.
    """

    a_dict = load_dict_from_file(basename)

    i = 0
    dash = '-' * 175

    for k in a_dict:

        error = basename.find('error')
        if (error == -1):
            id_check = "Check" + ' ' + str(i + 1).zfill(4)
        else:
            id_check = k

        v_check_dict_i = a_dict[id_check]
        link_i = v_check_dict_i['Link']
        date_i = v_check_dict_i['Date']
        status_i_dict = v_check_dict_i['Status']
        code_i = int(status_i_dict['Code'])
        meaning_i = status_i_dict['Meaning']
        type_i = status_i_dict['Type']

        no_color = '\x1b[0m  '
        red_color = ' \x1b[0;37;41m'
        green_color = ' \x1b[1;32;40m'
        cyan_color = ' \x1b[0;37;44m'
        white_color = ' \x1b[1;30;47m'

        if (i == 0):
            print(dash)
            print(' {:^6s}{:^16s}{:^28}{:<108s}'.format(
                'Status', 'Type', 'Date', 'Link'))
            print(dash)

        if(code_i < 300):
            code_i = green_color + str(code_i) + no_color
            type_i = green_color + type_i + no_color

        elif(code_i >= 300 and code_i < 400):
            code_i = cyan_color + str(code_i) + no_color
            type_i = cyan_color + type_i + no_color

        elif(code_i == 999):
            code_i = '---'
            code_i = white_color + str(code_i) + no_color
            type_i = white_color + type_i + no_color

        else:
            code_i = red_color + str(code_i) + no_color
            type_i = red_color + type_i + no_color
        print(' {:^6s}{:^16s}{:<28s}{:<104s}'.format(
                code_i, type_i, date_i, link_i))
        i += 1


def print_menu_results(result_filename):
    """
    Purpose:   Print a menu to select how to show results.
    Arguments: - None.
    Returns:   - option (char): The option selecter by the user.
    """
    clear_screen = __import__('os_functions').clear_screen

    opt = ''
    while (opt != 'R' or opt != 'r' or opt != 'A' or opt != 'a'):
        print('\n\t----------------------------------')
        print('\t(A) Show all the results')
        print('\t(R) Show a resume of the errors')
        print('\t(B) Go back')
        print('\t(R) Show other results')
        print('\t(M) Send mail with this results')
        print('\t(Q) Quit')
        print('\t----------------------------------')
        opt = input('\tType your choise and hit enter: [ ]\b\b')

        clear_screen()
        if (opt == 'r' or opt == 'R'):
            print_board(result_filename + '--error_dict')

        if (opt == 'a' or opt == 'A'):
            print_board(result_filename)

        if (opt == 'q' or opt == 'Q'):
            exit(1)

        if (opt == 'b' or opt == 'b'):
            return opt


def save_dict_to_file(dict, name):
    """
    Purpose:   Saves a "dict" into a JSON file with the "name" provided.
    Arguments: - Dict (Dict): Dict generated by the checking process.
    Returns:   - Nothing.
    """
    path = 'filestorage/'

    slash = name.find('/')
    if (slash != -1):
        name = name[0:slash]

    filename = path + name + '.json'
    f = open(filename, 'w')
    objects = json.dumps(dict, indent=4, skipkeys=True, sort_keys=True)
    f.write(objects)
    f.close()


def load_dict_from_file(name):
    """
    Purpose:   Gets a "dict" from a JSON file with the "name" provided.
    Arguments: - name (str): Name of the JSON file.
    Returns:   - dict (dict): dictionary object.
    """
    path = 'filestorage/'
    filename = path + name + '.json'
    f = open(filename, 'r')
    data = f.read()
    dict = json.loads(data)
    f.close()
    return (dict)


def print_progress_bar(count, total, status=''):
    """
    Purpose:   Prints a progress bar while checking the links.
    Arguments: - count (int): Counter of iterations.
               - total (int): Max number of iterations.
               - status (str): Text to be show while executing.
    Returns:   - Nothing.

    source: https://gist.github.com/vladignatyev/06860ec2040cb497f0f3
    Author: Vladimir Ignatyev
    """

    bar_len = 50
    filled_len = int(round(bar_len * count / float(total)))
    percents = round(100.0 * count / float(total), 3)
    mark = '■'
    bar = mark * filled_len + '∙' * (bar_len - filled_len)
    sys.stdout.write('%s\t[%s] %s%s\r' % (status, bar, percents, '%'))
    sys.stdout.flush()


def request_one(link):
    """
    Purpose:   Obtain a HTTP Status Codes by request of a valid link.
    Arguments: - link (str): link to be used in the request.
    Returns:   - HTTP Status Codes or 999 if not connection.
    """

    try:
        code = requests.get(link).status_code
        return ([code, str(now_stamp())])

    except:
        return ([999, str(now_stamp())])


def pooling_list(link_list):
    """
    Purpose:   Pooling to process multiple request in paralel.
               In this case, the funtion to be applied is request_one.
    Arguments: - link_list (lst): link list to be procesed in paralel.
    Returns:   - results (lst) A list with the request codesn.
    """


    if (link_list is not None):
        list_len=len(link_list)

        tam=250 if list_len > 250 else list_len
        with Pool(tam) as p:
            result=p.map(request_one, link_list)
            print_progress_bar(len(result), list_len, 'Checking:')

        p.close()
        return (result)
