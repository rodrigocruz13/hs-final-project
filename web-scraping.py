...
def get_link_list(fixed_url_list):
    ...
    for fixed_url in fixed_url_list:
        ...
        if(fixed_url not in unique_links):
...
def trim_string(a_string, a_text):
    ...
    Returns:   - A trimmed string if the text is found or the same string if not.
    """
...
def check_all(url, link_list):
    ...
    print("\nCheckin links.  Time in seconds : ", (end-start))
...
"""
def pooling_list(url, link_list):

    if (link_list is not None):
        list_len = len(link_list)

        with Pool(list_len) as p:
            results = p.map(request_one, (link_list))
